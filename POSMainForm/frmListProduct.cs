...
namespace POSMainForm
{
    public partial class frmListProduct : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        DialogResult btnYesNo = MessageBox.Show("Are you sure?", "Delete product", MessageBoxButtons.YesNo);

                        if(btnYesNo == DialogResult.Yes)
                        if(SQLConn.cmd.ExecuteNonQuery() > 0)
                        {
                            LoadProducts("");
            ...
            {
                Interaction.MsgBox("Please select record to delete", MsgBoxStyle.Exclamation, "Deletes");
                return;
            }
        }
        ...
        {
            barcodePopup barcodePopup = new barcodePopup();
            barcodePopup.ShowDialog();
            if (barcodeNumber > 0)
            {
                LinkedList<ListViewItem> itemsList = new LinkedList<ListViewItem>();
                foreach (ListViewItem selectedItem in ListView1.SelectedItems)
                {
                    itemsList.AddFirst(selectedItem);
                }

                frmBarcode frmBarcode = new frmBarcode(itemsList);
                frmBarcode.ShowDialog();
            }
            else
            {
                MessageBox.Show("Enter a valid number greater than 0.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}
